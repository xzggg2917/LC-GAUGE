...
const MethodsPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    console.log(`??? MethodsPage 第 ${currentCount} 次显示`)
            ...
            try {
              ...
              if (hasValidConfig) {
                ...
                calculateFullScoreAPI({ silent: true }).catch(err => {
                  console.warn('?? 自动重新计算评分失败（已忽略）:', err)
                })
              } else {
                console.log('?? 跳过自动评分计算（配置数据不完整）')
              }
            } catch (err) {
              console.warn('?? 检查配置数据失败:', err)
            }
          }, 800)
          
        } else {
          console.log('  ?? 存储中没有factors数据，清空试剂列表')
          setFactorsData([])
    ...
    const loadAllData = async () => {
      ...
      if (methodsData) {
        ...
        if (methodsData.weightSchemes) {
          ...
          console.log('?? [loadAllData] weightSchemes不存在，设置默认值');
        ...
        console.log('?? [loadAllData] methodsData不存在，设置默认值');
        setCustomWeights({})
      }
      
      // ?? 能耗数据不在此处加载，由独立的useEffect管理（避免被刷新覆盖）
      
      if (methodsData) {
        // ?? 关键：加载已有的Mobile Phase A/B到state（创建深拷贝）
      ...
      const verifyData = async () => {
        ...
        if (methodsData?.weightSchemes) {
          ...
          console.log('?? [verifyData] weightSchemes不存在，设置默认值');
          setSafetyScheme('PBT_Balanced')
          setHealthScheme('Absolute_Balance')
          setEnvironmentScheme('PBT_Balanced')
          setStageScheme('Balanced')
    ...
    const handleGradientDataUpdated = async () => {
      ...
      if (gradientData) {
        ...
        calculateFullScoreAPI({ silent: true }).catch(err => {
          console.warn('?? 梯度数据更新后自动计算评分失败:', err)
        })
      }
    }
    ...
    const checkGradientDataOnLoad = async () => {
      ...
      if (gradientData) {
        ...
        if (Array.isArray(gradientData) || !gradientData.calculations) {
          console.warn('?? 打开的文件缺少gradient calculations数据')
    ...
    const handleFileDataChanged = async (e: Event) => {
      ...
      if (methodsData?.weightSchemes) {
        ...
        // 如果是新文件，重置为默认值
        console.log('?? [文件切换] 新文件或没有weightSchemes，重置为默认权重方案')
        setSafetyScheme('PBT_Balanced')
        setHealthScheme('Absolute_Balance')
        setEnvironmentScheme('PBT_Balanced')
        setStageScheme('Balanced')
      ...
      console.log('?? 文件切换完成，等待用户手动触发评分计算')
  ...
  useEffect(() => {
    ...
    if (isSyncingFromContext.current) {
      console.log('?? MethodsPage: 正在从Context同步，跳过自动保存')
      return
    }
    ...
    if (isInitialMount.current) {
      ...
      console.log('?? MethodsPage: 首次挂载，跳过自动保存')
    ...
    const saveData = async () => {
      ...
      if (isInitialMount.current) {
        console.log('?? MethodsPage: 跳过初始挂载时的更新')
        isInitialMount.current = false
      ...
      if (lastLocalData.current === currentLocalDataStr) {
        console.log('?? MethodsPage: 本地数据未变化，跳过Context更新')
        return
      }
  ...
  const recalculateGradientCalculations = async (phaseA: Reagent[], phaseB: Reagent[]) => {
    try {
      const gradientData = await StorageHelper.getJSON(STORAGE_KEYS.GRADIENT)
      if (!gradientData) {
        console.log('?? 没有gradient数据，跳过重新计算')
        return
      }
      if (!gradientData.calculations) {
        console.log('?? gradient数据没有calculations，跳过重新计算')
        return
      }
  ...
  const preTreatmentChartData = React.useMemo(() => {
    ...
    if (!factorsData || factorsData.length === 0) {
      console.log('  ?? factorsData 未加载，跳过图表计算')
      return chartData
    }
    ...
    preTreatmentReagents.forEach(reagent => {
      ...
      if (!factor) {
        console.log(`  ?? 找不到试剂 ${reagent.name} 的factor数据`)
        return
      }
  ...
  const calculatePhaseChartData = async (phaseType: 'A' | 'B') => {
    ...
    try {
      ...
      if (gradientData.isValid === false || gradientData.calculations === null) {
        console.log('  ?? Gradient数据无效（流速为0），返回特殊标记')
      ...
      phaseData.components.forEach((component: any) => {
        ...
        if (!factor) {
          console.log(`  ?? 找不到试剂 ${component.reagentName} 的factor数据`)
          return
        }
  ...
  useEffect(() => {
    const loadPhaseAChartData = async () => {
      ...
      if (factorsData.length === 0 || !gradientCalculations) {
        console.log('  ?? 数据不完整，清空 Phase A 图表')
        setPhaseAChartData([])
        return
      }
  ...
  useEffect(() => {
    const loadPhaseBChartData = async () => {
      ...
      // 数据加载完成但不完整，才清空图表
      if (factorsData.length === 0 || !gradientCalculations) {
        console.log('  ?? 数据不完整，清空 Phase B 图表')
        setPhaseBChartData([])
        return
      }
  ...
  const calculateFullScoreAPI = async (options?: { silent?: boolean; overrides?: any }) => {
    ...
    try {
      ...
      if (prepReagents.length > 0) {
        ...
        console.log('  ?? 没有前处理试剂，使用Water作为占位符')
  ...
  useEffect(() => {
    ...
    if (!isAutoCalcInitialized.current) {
      console.log('?? 自动计算: 跳过初始挂载')
      isAutoCalcInitialized.current = true
      return
    }
    
    // ?? 如果数据正在加载中，跳过自动计算（等数据加载完成后再触发）
    if (isDataLoading) {
      console.log('?? 自动计算: 数据加载中，跳过')
      return
    }
        ...
        <Row gutter={16}>
          <Col span={8}>
            ...
            <Select style={{ width: '100%', marginBottom: 12 }} value={safetyScheme} onChange={(value) => { 
              console.log('?? 安全因子权重方案变化:', safetyScheme, '->', value); 
              if (value === 'Custom') {
                setCustomWeightType('safety');
                setCustomWeightModalVisible(true);
                return;
          ...
          <Col span={8}>
            ...
            <Select style={{ width: '100%', marginBottom: 12 }} value={healthScheme} onChange={(value) => { 
              console.log('?? Health Factor权重方案变化:', healthScheme, '->', value); 
              if (value === 'Custom') {
                setCustomWeightType('health');
                setCustomWeightModalVisible(true);
                return;
            ...
            <Select style={{ width: '100%', marginBottom: 12 }} value={environmentScheme} onChange={(value) => { 
              console.log('?? Environmental Factor权重方案变化:', environmentScheme, '->', value); 
              if (value === 'Custom') {
                setCustomWeightType('environment');
                setCustomWeightModalVisible(true);
                return;
          ...
          <Col span={8}>
            ...
            <Select style={{ width: '100%', marginBottom: 12 }} value={stageScheme} onChange={(value) => { 
              console.log('?? Stage权重方案变化:', stageScheme, '->', value); 
              if (value === 'Custom') {
                setCustomWeightType('stage');
                setCustomWeightModalVisible(true);
                return;
            ...
            <Select style={{ width: '100%' }} value={finalScheme} onChange={(value) => { 
              console.log('?? 最终汇总权重方案变化:', finalScheme, '->', value); 
              if (value === 'Custom') {
                setCustomWeightType('final');
                setCustomWeightModalVisible(true);
                return;
