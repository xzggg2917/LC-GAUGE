...
```
Layer 0: 质量计算 (Mass Calculation)
   ↓
Layer 1: 小因子归一化 (9个小因子，0-100分制)
   ↓
...
| curve_8 | 0.310 | 强缓入急出 |
| curve_9 | 0.690 | 强急入缓出 |
| curve_10 | 0.333 | S型（前半段慢）|
| curve_11 | 0.667 | S型（后半段慢）|
...
### Q11: 梯度程序中的非直线曲线如何计算？
**A**: 系统实现了精确的曲线积分。对于linear（直线），使用简单平均（积分系数0.5）；对于curve_6到curve_11等非线性曲线，使用对应的积分系数（0.310到0.690之间）。这确保了不同曲线类型下试剂用量计算的准确性。
